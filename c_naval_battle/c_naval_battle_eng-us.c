	...
	for (i=0;i<8;i++) {
		...
		for (j=0;j<8;j++) {
			if(board[i][j] == 9) {
				...
				printf(ANSI_COLOR_CYAN " W " ANSI_COLOR_RESET);
				printf("|");
			} else {
				printf(" ? |");
			}
...
void move (int x, int y, int (*board)[8], int *ship) {
	...
	switch (board[x][y]) {
		...
		case 5:
			...
			break;
		case 9:
			printf("You've already hit that square!\n");
			break;
		case 10:
			printf("You've already hit that square!\n");
			break;
...
int main(void) {
	...
	for (i=0;i<40;i++) {
		print_board(board);
		printf("\n\n--- Play ---\n\n");
		printf("Insert the first coordinate [1-8]: \n[Line] --> ");
		scanf("%d", &x);
		
		printf("Insert the second coordinate [1-8]: \n[Column] -->");
		scanf("%d", &y);
		
		if (x > 8 || y > 8) {
			printf("\nValue too high, insert again!\n");
			sleep(1);
		} else if (x < 1 || y < 1) {
			printf("\nValue too low, insert again!\n");
			sleep(1);
		} else {
